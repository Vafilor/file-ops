...
from file_ops.actions.unique_names import (
    get_path_total_stem,
    get_unique_name_changes_for_paths,
...
def test_path_with_total_stem() -> None:
    assert path_with_total_stem(Path("path/hello.txt.wow"), "world") == Path(
        "path/world.txt.wow"
    )
    assert path_with_total_stem(Path("path/hello.txt"), "world") == Path(
        "path/world.txt"
    )
...
def test_generate_unique_name_changes_for_paths_single() -> None:
    result = list(get_unique_name_changes_for_paths([Path("hello")]))
...
def test_generate_unique_name_changes_for_paths_multiple() -> None:
    result = list(
        get_unique_name_changes_for_paths(
            [Path("hello"), Path("world.txt"), Path("cake.png")]
        )
    )
...
def test_generate_unique_name_changes_for_paths_subdirectory() -> None:
    result = list(
        get_unique_name_changes_for_paths([Path("hello.txt"), Path("sub/hello.txt")])
    )
...
def test_generate_unique_name_changes_for_paths_preserve_for_extension() -> None:
    result = list(
        get_unique_name_changes_for_paths(
            [Path("hello"), Path("hello.txt"), Path("hello.txt.zip")]
        )
    )
    ...
    assert (
        get_path_total_stem(first.new_path)
        == get_path_total_stem(second.new_path)
        == get_path_total_stem(third.new_path)
    )
